...
def call_openai_analysis(coin_data: dict) -> str:
    ...
    prompt = f"""
   «تو یک تحلیلگر حرفه‌ای بازار کریپتو هستی. هر زمان یک ارز دیجیتال از تو خواسته شد، با توجه به اطلاعات که در انتها بهت داده میشه لطفاً بر اساس ساختار زیر یک معرفی کامل، روان، ساده و قابل فهم در حد 350 تا 450 کلمه ارائه بده. همچنین  قبل از استفاده از نام نماد از کلمه رمزارز استفاده کن متن باید کاملاً منظم و بخش‌بندی‌شده باشد و هیچ‌گونه پیشنهاد خرید یا فروش نداشته باشد.
۱. معرفی کوتاه
