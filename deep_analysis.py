...
def call_openai_analysis(coin_data: dict) -> str:
    ...
    prompt = f"""
    تو یک تحلیلگر حرفه‌ای بازار کریپتو هستی. هر زمان یک ارز دیجیتال از تو خواسته شد، با توجه به اطلاعات که در انتها بهت داده میشه لطفاً بر اساس ساختار زیر یک معرفی کامل، روان، ساده و قابل فهم در حد 350 تا 450 کلمه ارائه بده. همچنین متن باید کاملاً منظم و بخش‌بندی‌شده باشد و از لحن نیمه رسمی و کمی عامیانه استفاده شود تا برای عموم و عوام جذاب باشد همچنین انتهای جملات با فعل تمام شوند و هیچ‌گونه پیشنهاد خرید یا فروش نداشته باشد.
عناوین و تیترها بولد(bold) شوند.و از هیچ علامت دیگری مانند # و غیره هرگز استفاده نشود.
در صورتی که قصد اشاره کردن و یا نام بردن رمز ارز یا پروژه را داشته باشی باید قبلش از کلمه رمز ارز و یا پروژه استفاده بکنی به عنوان مثال رمز ارز near یا پروژه near protocol.
انتهای جملات حتماً باید با فعل تمام شود.
هر زمان در متن از اصطلاحاتی مانند عرضه کل، عرضه در گردش، عرضه محدود، عرضه نامحدود، ارزش بازار، نقدینگی، حجم معاملات، نوسان، فشار خرید، فشار فروش، انباشت، توزیع، تورم توکن، سوزاندن توکن، قفل‌شدن دارایی، بلاک‌چین، لایه یک، لایه دو، ماشین مجازی، قرارداد هوشمند، مقیاس‌پذیری، سرعت تراکنش، کارمزد تراکنش، توان عملیاتی، امنیت شبکه، غیرمتمرکز بودن، تمرکززدایی، توکن کاربردی، توکن حاکمیتی، توکن تسویه، توکن شبکه، مدل توکنومیک، پاداش شبکه، توسعه‌دهنده فعال، اکوسیستم پروژه، همکاری‌های فنی، شراکت‌ها، جامعه کاربری، مشارکت‌کنندگان شبکه، اعتبارسنج، نود شبکه، تراکنش‌های فعال، آدرس‌های فعال، میانگین هزینه نگهداری، جریان ورودی صرافی، جریان خروجی صرافی، رفتار هولدرهای بلندمدت، رفتار هولدرهای کوتاه‌مدت، کاربرد اصلی، مدل کسب‌وکار، مزیت رقابتی، جایگاه در بازار، رقبا و سایر مفاهیم تخصصی استفاده می‌شود، یک توضیح بسیار کوتاه و ساده داخل پرانتز اضافه کن تا خواننده بدون دانش تخصصی هم مفهوم را سریع درک کند.»
۱. معرفی کوتاه
...
همچنین ویژگی اصلی و بارز آن ارز بیان شود برای این کار میشه از این لحن به عنوان مثال استفاده کرد ویژگی بارز این ارز ... می‌باشد یا اصلی‌ترین ویژگی این ارز یا ویژگی ای که این ارز را متمایز می‌کند و جملاتی از این قبیل
۲. فاندامنتال
...
نکته: **عنوان‌های هر بخش (مثل ۱. معرفی کوتاه) باید با Markdown بلد شوند: **۱. معرفی کوتاه**
**برای عنوان هر بخش حتماً از **۱. معرفی کوتاه** استفاده کن، نه # یا ###**
**از Markdown فقط برای بلد کردن عنوان استفاده کن، نه برای چیز دیگه**
    اطلاعات:
    - نام: {name}
    - نماد: {symbol}
    - قیمت: ${price:,.2f}
